...
## ⚠️ 重要：文件权限要求（避免升级写入失败）

系统升级时需要通过 PHP 进程（通常是 `www` 或 `www-data` 用户）**覆盖写入站点文件**。
如果站点文件属主不是 PHP 运行用户（例如用 root 上传/解压了文件），升级会报「N 个文件写入失败」。

**部署时必须确保：**

```bash
# 宝塔面板环境（PHP 运行用户通常是 www）
chown -R www:www /www/wwwroot/你的站点目录
chmod -R 755 /www/wwwroot/你的站点目录

# 或 Debian/Ubuntu 环境（PHP 运行用户是 www-data）
chown -R www-data:www-data /var/www/你的站点目录
chmod -R 755 /var/www/你的站点目录
```

> 💡 自 v1.7.1 起，升级程序已内置**健壮写入**：即使个别文件属主异常，也会自动尝试修复后写入；
> 并在站点目录整体不可写时，于升级前给出明确的修复命令提示（而不是升级到一半才报错）。
