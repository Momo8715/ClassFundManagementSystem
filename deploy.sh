...
#
# 用法：
#   ./deploy.sh v2.0          # 大版本更新（一般不用）
#   ./deploy.sh v1.5.0        # 新功能（x.xx.0）
#   ./deploy.sh v1.5.3        # 修复 bug / 体验优化（x.xx.xx）
#   ./deploy.sh v1.5.0 --bt   # 只上传到宝塔（跳过 GitHub 发布）
#
# 流程：
...
if [ -z "$VERSION" ]; then
  ...
  echo "   示例: ./deploy.sh v1.5.0（新功能 x.xx.0）  ./deploy.sh v1.5.3（修复/优化 x.xx.xx）  ./deploy.sh v2.0（大版本，一般不用）"
  exit 1
fi
