...
#
# 用法：
#   ./deploy.sh v2.0          # 大版本更新并部署到宝塔
#   ./deploy.sh v1.4          # 发布新功能并部署到宝塔
#   ./deploy.sh v1.4.1        # 修复 bug / 体验优化并部署到宝塔
#   ./deploy.sh v1.4 --bt     # 只上传到宝塔（跳过 GitHub 发布）
#
# 流程：
...
if [ -z "$VERSION" ]; then
  ...
  echo "   示例: ./deploy.sh v2.0（大版本）  ./deploy.sh v1.4（新功能）  ./deploy.sh v1.4.1（bug修复/优化）"
  exit 1
fi
